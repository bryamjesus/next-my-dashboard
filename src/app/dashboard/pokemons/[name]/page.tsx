...
export async function generateStaticParams() {
  ...
  const statis152Pokemons = data.results.map((pokemon) => ({
    name: pokemon.name,
  }));

  return statis152Pokemons.map(({ name }) => ({ name: name }));
}
...
// ✅ LINK SOLUCION : https://nextjs.org/docs/app/api-reference/functions/generate-metadata#generatemetadata-function
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    ...
    return {
      ...
      description: `Pokémon ${pokemon.name}`,
    };
  } catch (error) {
    console.log(error);
    return {
